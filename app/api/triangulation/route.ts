...
import { fetchTriangulation, type TriFilters } from "@/lib/dhis2/triangulation";
...
export async function GET(req: NextRequest) {
  ...
  const level = sp.get("level") === "zs" ? "zs" : "as";
  const filters: TriFilters = {
  ...
  try {
    const bundle = await fetchTriangulation(level, filters, { force });
