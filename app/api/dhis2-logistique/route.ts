...
import { fetchDhis2Logistique, type LogistiqueFilters } from "@/lib/dhis2/logistique";
...
export async function GET(req: NextRequest) {
  ...
  const filters: LogistiqueFilters = {
    antenne: sp.get("antenne"),
    months: multi(sp, "months"),
  };
  const force = sp.get("force") === "1";
  try {
    const bundle = await fetchDhis2Logistique(filters, { force });
